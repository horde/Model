...
class Horde_Core_Ui_VarRenderer_Html extends Horde_Core_Ui_VarRenderer
{
    ...
    {
        return sprintf(
            '<input type="text" name="%s" id="%s" size="15" value="%s" %s%s />',
            htmlspecialchars($var->getVarName()),
            $this->_genID($var->getVarName(), false),
            htmlspecialchars($var->getValue($vars)),
            $var->isDisabled() ? ' disabled="disabled" ' : '',
    ...
    protected function _renderVarInput_image($form, &$var, &$vars)
    {
        ...
        if (!empty($image['img'])) {
             ...
             */
            /* Rotate 270. */
            $html .= Horde::link('#', Horde_Core_Translation::t("Rotate Left"), '', '', 'showImage(\'' . $img->copy()->add(['a' => 'rotate', 'v' => '270']) . '\', \'_p_' . $varname . '\', true);') . Horde::img('image/rotate-270.png', Horde_Core_Translation::t("Rotate Left")) . '</a>';
             ...
             */
            /* Rotate 180. */
            $html .= Horde::link('#', Horde_Core_Translation::t("Rotate 180"), '', '', 'showImage(\'' . $img->copy()->add(['a' => 'rotate', 'v' => '180']) . '\', \'_p_' . $varname . '\', true);') . Horde::img('image/rotate-180.png', Horde_Core_Translation::t("Rotate 180")) . '</a>';
             ...
             */
            /* Rotate 90. */
            $html .= Horde::link('#', Horde_Core_Translation::t("Rotate Right"), '', '', 'showImage(\'' . $img->copy()->add(['a' => 'rotate', 'v' => '90']) . '\', \'_p_' . $varname . '\', true);') . Horde::img('image/rotate-90.png', Horde_Core_Translation::t("Rotate Right")) . '</a>';
    ...
    protected function _renderVarInput_monthdayyear($form, &$var, &$vars)
    {
        ...
        var_dump($date);
        $html = sprintf(
            '<input type="text" name="%s" id="%s" size="10"%s>',
            htmlspecialchars($var->getVarName()),
    ...
    protected function _renderVarInput_obrowser($form, &$var, &$vars)
    {
        ...
        if ($GLOBALS['browser']->hasFeature('javascript')) {
             ...
             * @see Horde_Deprecated::img()
             */
            $html .= Horde::link($GLOBALS['registry']->get('webroot', 'horde') . '/services/obrowser/', Horde_Core_Translation::t("Select an object"), '', '_blank', 'obrowserWindow = ' . Horde::popupJs($GLOBALS['registry']->get('webroot', 'horde') . '/services/obrowser/', ['urlencode' => true]) . 'obrowserWindowName = obrowserWindow.name; return false;') . Horde::img('tree/leaf.png', Horde_Core_Translation::t("Object")) . "</a>\n";
        }
